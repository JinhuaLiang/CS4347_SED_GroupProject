# The code was designed for Weakly-Supervised Sound Event Detection in CS4347.
# Its framework was modified from Qiuqiang Kong's code: 
# https://github.com/qiuqiangkong/sound_event_detection_dcase2017_task4
import os
import sys
...
def train(args):
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 1000 == 0 and iteration > resume_iteration):# or (iteration == 0):
            ...
            (data_type, data_loader, reference_csv_path) = ('test', test_loader, test_reference_csv_path)

            # Calculate tatistics
            (statistics, _) = evaluator.evaluate(data_loader, reference_csv_path, tmp_submission_path)
...
def inference_prob(self):
    ...
    """Once the valuation dataset is released, 
       one can uncomment codes regarding valuation."""

    test_reference_csv_path = os.path.join(dataset_dir, 'metadata', 
        'groundtruth_strong_label_testing_set.csv')
        
    evaluate_reference_csv_path = None
    """The groundtruth of valuation dataset will not provided"""
    ...
    (data_type, data_loader, reference_csv_path) = ('evaluate', evaluate_loader, evaluate_reference_csv_path)
    # (data_type, data_loader, reference_csv_path) = ('test', test_loader, test_reference_csv_path)
